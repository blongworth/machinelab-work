...
# Need to group by upload session
count <- adv_data |> 
  pull(count)

last_count <- count[(length(count)-150):length(count)]
last_count
count_missing(last_count)
```
### Plot missingness
...
```{r}
adv_data <- adv_data |> 
  mutate(missing = if_else(count > lag(count),
                           count - 1L - lag(count),
                           255L + count - lag(count),
                           ))
