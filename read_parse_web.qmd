...
time_offset = as.POSIXct("2024-03-06 08:00:00") - as.POSIXct("2019-01-01 00:00:00")
...
post_times <- lecs_post_times(df) |> 
  mutate(timestamp = timestamp + time_offset)
...
adv_data_mean <- adv_data_qc |> 
  select(send, missing, timestamp, pressure, ph_counts, temp, DO_percent, pH) |> 
  group_by(send) |> 
  summarise(across(everything(), 
                   list(mean = ~ mean(.x, na.rm = TRUE), 
                        sd = ~ sd(.x, na.rm = TRUE))),
