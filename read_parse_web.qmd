...
```{r}
df_raw <- read_html("https://gems.whoi.edu/LECS_data/") |> 
  html_node("table") |> 
  html_table()
```
...
met <- df |> 
  ...
  separate(data, 
           ...
           sep = ',') |> 
  mutate(across(4:9, as.integer),
         across(10:12, as.numeric)) |> 
...
status <- df |> 
  ...
  separate(data, 
           ...
           sep = ',') |>
  mutate(across(4:15, as.integer),
         across(16:24, ~ as.numeric(.x) * .1),
