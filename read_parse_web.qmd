...
```{r}
#| eval: true
adv_data <- make_lecs_ts_2(adv_data, status)
lps <- NA
```

```{r}
#| eval: false
bm <- bench::mark(adv_data <- make_lecs_ts_2(adv_data, status))
lps <- nrow(adv_data) / bm$median[[1]]
bm
```
...
```{r}
adv_data_qc |> 
  ggplot(aes(timestamp, ph_counts)) +
  geom_point()
```
