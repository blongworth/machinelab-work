...
met_qc <- lecs_clean_met(met)

status <- lecs_status_data(df)
status_qc <- lecs_clean_status(status)

adv_data <- lecs_adv_data(df, rinko_cals) 
adv_data_qc <- lecs_clean_adv_data(adv_data) |> 
...
Filter bad data (now done above)
...
These plots show change in mean spectral power from 0.1 to 1 Hz over time.

temp

```{r}
adv_data_qc |> 
  filter(!(send %in% c(42, 212, 346))) |> 
  arrange(timestamp) |> 
  group_by(send) |> 
  filter(line > 30) |>
  mutate(resp = calc_response(temp, timestamp, 16)) |> 
  select(send, resp) |> 
  left_join(post_times) |> 
  ggplot(aes(timestamp, resp)) + geom_line() +
  scale_y_log10() +
  ggtitle("Temperature response",
          subtitle = "mean spectral power between 0.1 and 1 Hz")
```
...
adv_data_qc |> 
  ...
  group_by(send) |> 
  filter(line > 30) |>
  mutate(resp = calc_response(pH, timestamp, 16)) |> 
  select(send, resp) |> 
  left_join(post_times) |> 
  ggplot(aes(timestamp, resp)) + geom_line() + 
  scale_y_log10() +
...
adv_data_qc |> 
  ...
  group_by(send) |> 
  filter(line > 30) |>
  mutate(resp = calc_response(DO, timestamp, 16, freq_range = c(0.1, 1))) |> 
  select(send, resp) |> 
  left_join(post_times) |> 
  ggplot(aes(timestamp, resp)) + geom_line() +
  scale_y_log10() +
  ggtitle("Oxygen response",
          subtitle = "mean spectral power between 0.1 and 1 Hz")
```

### Flux

Per send fluxes. Sends are 500 lines, so roughly 30 seconds of data.

```{r}
adv_data_qc |> 
  filter(!(send %in% c(356))) |> 
  arrange(timestamp) |> 
  group_by(send) |> 
  filter(line > 30) |>
  mutate(resp = calc_flux(DO, w, timestamp, 16)) |> 
  select(send, resp) |> 
  left_join(post_times) |> 
  ggplot(aes(timestamp, resp)) + geom_line() +
  scale_y_log10() +
