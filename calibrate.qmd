...
```{r}
ds <- ds |> 
  mutate(ph_group = if_else(time > "2023-12-22 06:45:00", 1L, 0L))
```
...
ds_cal <- df_cal |> 
  ...
  arrow_table() 
ds_cal |> 
  write_dataset(paste0(data_dir, "lecs_adv_data_all_filtered_calibrated.parquet"))
```
...
adv_data_mean <- df_cal |>
  select(-c(ph_group, ph_i, ph_c, ph_t)) |> 
  mutate(timestamp = date_group(timestamp, "hour")) |> 
  group_by(timestamp) |> 
  #group_by(year, month, day, hour) |> 
  summarise(across(everything(), 
                   ~ mean(.x, na.rm = TRUE))) |> 
  ungroup()
fwrite(adv_data_mean, 
       paste0(data_dir, "lecs_adv_data_all_filt_cal_hourly.csv"))
write_dataset(adv_data_mean, 
...
Works in Arrow! But can't handle timezone. TZ set later, confirm correct
...
write_dataset(adv_data_mean, paste0(data_dir, "all_minute.parquet"))
```
