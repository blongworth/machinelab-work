...
flux <- read_parquet(file.path(data_dir, "lecs_flux_4hz_rot_fl5.parquet/part-0.parquet"))
```
...
```{r}
o2 <- ggplot(flux_hf, aes(hour, Flux3cpsdLowStor_mean)) + 
  geom_line() +
  #geom_pointrange(aes(ymin = flux3o2_mean - flux3o2_sd,
  #                    ymax = flux3o2_mean + flux3o2_sd)) +
  geom_hline(yintercept = 0) +
...
par <- ggplot(flux_hf, aes(hour, par_mean)) +
  geom_line()
...
par <- ggplot(flux_h, aes(hour, par_mean, color = per)) +
  geom_line()
